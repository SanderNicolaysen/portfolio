...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        className={`fixed z-20 w-full top-0 left-0 h-20 items-center flex px-6 justify-between backdrop-blur text-white text-2xl bg-darkGray-500 dark:bg-secondary`}
      >
        ...
        <ul
          className={`${
            isOpen ? 'flex flex-col items-center justify-around h-screen' : 'hidden md:flex md:items-center'
          }`}
        >
          <li className="">
      ...
      <div className={`${isOpen ? 'block' : 'hidden'} bg-darkGray-500 h-screen`}></div>
    </>
  )
}
